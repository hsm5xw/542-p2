...
class net:   
    ...
    def deconvlayers(self):
        ...
        with tf.name_scope('deconv1') as scope:
            self.deconv1 = tf.image.resize_bilinear(self.conv8,[500,500])
        #    kernel = tf.Variable(tf.truncated_normal([64, 64, 21, 21],
        #                                                   dtype=tf.float32,
        #                                                  stddev=1e-1), name='weights')
        #    #kernel = tf.zeros([128, 128, 21, 21], dtype=tf.float32, name='weights')
        #    deconv = tf.nn.conv2d_transpose(self.conv8, kernel, [self.batch_size, 500, 500, 21], [1, 32, 32, 1], padding='SAME')
