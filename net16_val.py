...
class net:   
    ...
    def load_weights(self, sess, weight_file=None, trainable_weight=None, update_weight=None):
        ...
        if update_weight is not None:
            weights = np.load(update_weight)['arr_0']
            for i, k in enumerate(weights):
                sess.run(self.parameters[i].assign(weights[i]))

    def Validation(self):
        prediction = tf.argmax(self.deconv1,axis=3)
        iu = tf.metrics.mean_iou(self.label, tf.to_float(prediction), 21, name='meanIU')
        self.mean_oiu = iu
        acc = tf.metrics.accuracy(self.label, tf.to_float(prediction), name = 'pixelAccuracy')
        self.p_acc = acc

        l = tf.reshape(self.label, [-1])
        p = tf.reshape(prediction,[-1])
        self.cm = tf.confusion_matrix(l,p, num_classes=21)

        correct_prediction = tf.equal(tf.to_float(prediction), self.label)
